...
def _redact_url_params(text):
    def _redact(m):
        return _QUERY_PARAM_PATTERN.sub(r'\1=***', m.group(0))
    return _URL_PATTERN.sub(_redact, text)
...
class Tee:
    ...
    def write(self, data):
        ...
        if self.log_file and not self.log_file.closed:
            self.log_file.write(_redact_url_params(data))
